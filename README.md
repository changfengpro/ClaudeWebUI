...
也可分开跑：终端 1 `node server.mjs`（后端，8787），终端 2 `./run-openwebui.sh`（Open WebUI，3000）。
...
在 Open WebUI 里给消息附加图片即可，后端会按各 CLI 原生方式转交（指纹不变）：

- Claude：经 `--input-format stream-json` 由 stdin 发一条含 `image`(base64) 块的 user 消息，与交互式发图的内部格式一致。
- Codex：图片落临时文件后用 `codex exec -i <file>` 附加，结束即清理。

仅支持 `data:` 形式的 base64 图片（Open WebUI 上传的图片即此形式）。
